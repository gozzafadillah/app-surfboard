        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4 mb-4">Edit Estimasi</h1>
                    <div class="card mb-4">
                        <div class="card-header" style="display:flex; justify-content: space-between">
                            <p>Form Estimasi</p>
                        </div>
                        <div class="card-body">
                            <form action="/dashboard/estimasi/{{ $id }}/edit" method="post">
                                @csrf
                                @method('put')
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="model_produk_id"
                                        ...
                                        @foreach ($modelProduk as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id == $estimasi->model_produk_id ? 'selected' : '' }}>
                                                {{ $item->model }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" type="month" name="bln_estimasi" id="bln_estimasi"
                                        value="{{ $estimasi->bulan_estimasi }}">
                                </div>
                                <div class="mb-3">
                                    <label for="jumlah" class="form-label">Jumlah Produksi</label>
                                    <input type="number" placeholder="0" class="form-control" id="jumlah" name="jumlah"
                                        value="{{ $estimasi->jumlah }}">
